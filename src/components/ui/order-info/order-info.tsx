...
export const OrderInfoUI: FC<OrderInfoUIProps> = memo(({ orderInfo }) => {
  ...
  const backgroundForLink = location.state?.background || location;

  return (
    <div className={styles.wrap}>
      <h2 className={`text text_type_digits-default pt-10 ${styles.number}`}>
        #{orderInfo.number}
      </h2>
        ...
        {Object.values(orderInfo.ingredientsInfo).map((item, index) => (
          <li className={`pb-4 pr-6 ${styles.item}`} key={index}>
            <Link
              ...
              state={{ background: backgroundForLink, nested: true }}
